...
shared_ptr<Mesh> Mesh::CreateMesh()
{
    ...
    //정점타입
    temp->vertexType = VertexType::TERRAIN;
    //도형정의
    temp->primitiveTopology = D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
    //정점당 크기
    temp->byteWidth = sizeof(VertexTerrain);


    //정점 갯수
    temp->vertexCount = 257 * 257;

    //인덱스 갯수
    temp->indexCount = 256 * 256 * 6;

    //정점 데이터 생성
    VertexTerrain* Vertex = new VertexTerrain[temp->vertexCount];
    ...
    for (int i = 0; i < 257; i++)
    {
        ...
        {
            int idx = i * 257 + j;
            Vertex[idx].position = Vector3(j, 0, -i);
            Vertex[idx].uv = Vector2(j / 256.0f, i / 256.0f);
            Vertex[idx].normal = Vector3(0, 1, 0);
            Vertex[idx].weights = Vector4(1,0,0,0);
        }
    }
    ...
    for (int i = 0; i < 256; i++)
    {
        for (int j = 0; j < 256; j++)
        {
            ...
            //              사각형인덱스  =  x인덱스 + y인덱스
            temp->indices[idx * 6 + 0] = 0 + j + 257 * i;
            temp->indices[idx * 6 + 1] = 1 + j + 257 * i;
            temp->indices[idx * 6 + 2] = 1 + j + 257 * (i + 1);

            temp->indices[idx * 6 + 3] = 0 + j + 257 * i;
            temp->indices[idx * 6 + 4] = 1 + j + 257 * (i + 1);
            temp->indices[idx * 6 + 5] = 0 + j + 257 * (i + 1);
...
void Mesh::RenderDetail(GameObject* root)
{
    InstanceEdit(root);
...
bool Mesh::LoadInstanceFile(string file)
{
    instanceFile = file;
...
void Mesh::InstanceEdit(GameObject* root)
{
    //내 인스턴스 갯수
    ...
    for (int i = 0; i < this->instanceCount; i++)
    {
        ...
        if (ImGui::TreeNode(instanceCount.c_str()))
        {
            ...
            Vector3 temp = s;
            if (ImGui::DragFloat3("scale", (float*)&temp, 0.05f))
            {
                s = temp;
            }
            temp = r / ToRadian;
            if (ImGui::DragFloat3("rot", (float*)&temp, 0.05f))
            {
                r = temp * ToRadian;
            }
            temp =t;
            if (ImGui::DragFloat3("pos", (float*)&temp, 0.05f))
            {
                t = temp;
            }

            if (ImGui::Button("ColiderCreate"))
            {
                GameObject* temp2 = GameObject::Create(instanceCount);
                temp2->collider = new Collider(ColliderType::OBOX);
                temp2->SetLocalPos(t);
                temp2->rotation = r;
                temp2->scale = s;
                root->AddChild(temp2);
