...
void GameObject::SaveObject(Xml::XMLElement* This, Xml::XMLDocument* doc)
{
	...
	if (shadowShader)
	{
		Xml::XMLElement* Shader = doc->NewElement("ShadowShader");
		This->LinkEndChild(Shader);
		Shader->SetAttribute("File", shadowShader->file.c_str());
	}
	if (enviromentShader)
	{
		Xml::XMLElement* Shader = doc->NewElement("EnviromentShader");
		This->LinkEndChild(Shader);
		Shader->SetAttribute("File", enviromentShader->file.c_str());
	}
	if (waterShader)
	{
		Xml::XMLElement* Shader = doc->NewElement("WaterShader");
		This->LinkEndChild(Shader);
		Shader->SetAttribute("File", waterShader->file.c_str());
	}
	if (collider)
	{
		//Collider
...
void GameObject::LoadObject(Xml::XMLElement* This)
{
	...
	if (component = This->FirstChildElement("Mesh"))
	{
		...
		if (type == ObType::Rain )
		{
			...
			dynamic_cast<Rain*>(this)->Reset();
		}
		else if (type == ObType::Pop)
		{
			mesh = make_shared<Mesh>();
			mesh->LoadFile("8.Billboard.mesh");
			dynamic_cast<Pop*>(this)->Reset();
		}
	...
	if (component = This->FirstChildElement("ShadowShader"))
	{
		file = component->Attribute("File");
		SafeReset(shadowShader);
		shadowShader = RESOURCE->shaders.Load(file);
		shadowShader->LoadGeometry();
	}
	if (component = This->FirstChildElement("EnviromentShader"))
	{
		file = component->Attribute("File");
		SafeReset(enviromentShader);
		enviromentShader = RESOURCE->shaders.Load(file);
		enviromentShader->LoadGeometry();
	}
	if (component = This->FirstChildElement("WaterShader"))
	{
		file = component->Attribute("File");
		SafeReset(waterShader);
		waterShader = RESOURCE->shaders.Load(file);
		waterShader->LoadGeometry();
	}
...
void Actor::LoadFile(string file)
{
	...
	if (type == ObType::Terrain)
	{
		((Terrain*)this)->UpdateStructuredBuffer();
	}
}
