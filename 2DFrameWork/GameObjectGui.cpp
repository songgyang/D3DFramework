...
bool GameObject::RenderHierarchy()
{
	...
	if (ImGui::TreeNode(name.c_str()))
	{
		...
		if (ImGui::BeginPopup("childName"))
		{
			...
			{
				AddChild(Rain::Create(childName));
			}
			ImGui::SameLine();
			if (ImGui::Button("Pop"))
			{
				AddChild(Pop::Create(childName));
			}
			ImGui::EndPopup();
		}
...
void GameObject::RenderDetail()
{
	...
	ImGui::Checkbox("rootMotion", &rootMotion);
	if (ImGui::BeginTabBar("MyTabBar"))
	{
		...
		if (ImGui::BeginTabItem("Mesh"))
		{
			if (mesh)
			{
				...
				if(root)
					mesh->RenderDetail(root);
				else
					mesh->RenderDetail(this);
			}
			else
			{
		...
		if (ImGui::BeginTabItem("ShadowShader"))
		{

			if (shadowShader)
			{
				ImGui::Text(shadowShader->file.c_str());
			}
			if (GUI->FileImGui("Load", "Load ShadowShader",
				".hlsl", "../Shaders"))
			{
				string path = ImGuiFileDialog::Instance()->GetCurrentPath();
				Utility::Replace(&path, "\\", "/");
				...
				shadowShader = RESOURCE->shaders.Load(path);
		...
		if (ImGui::BeginTabItem("EnviromentShader"))
		{

			if (enviromentShader)
			{
				ImGui::Text(enviromentShader->file.c_str());
			}
			if (GUI->FileImGui("Load", "Load EnviromentShader",
				".hlsl", "../Shaders"))
			{
				string path = ImGuiFileDialog::Instance()->GetCurrentPath();
				Utility::Replace(&path, "\\", "/");
				...
				enviromentShader = RESOURCE->shaders.Load(path);
		...
		if (ImGui::BeginTabItem("WaterShader"))
		{

			if (waterShader)
			{
				ImGui::Text(waterShader->file.c_str());
			}
			if (GUI->FileImGui("Load", "Load WaterShader",
				".hlsl", "../Shaders"))
			{
				string path = ImGuiFileDialog::Instance()->GetCurrentPath();
				Utility::Replace(&path, "\\", "/");
				...
				waterShader = RESOURCE->shaders.Load(path);
